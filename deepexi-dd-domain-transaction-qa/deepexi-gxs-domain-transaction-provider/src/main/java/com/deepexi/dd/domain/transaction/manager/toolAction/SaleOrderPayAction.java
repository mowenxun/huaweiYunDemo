...
/**
 * @ClassName : SaleOrderPayAction
 * @Description : 网批订单付款环节
 * @Author : yuanzaishun
 * @Date: 2020-08-14 19:40
 */
@Component
@ActionCodeAnnotation(listType = ListTypeEnum.SalesOrder,code = "BuyerPayment")
public class SaleOrderPayAction extends SaleOrderToolAction {
   Logger logger= LoggerFactory.getLogger(SaleOrderPayAction.class);
    ...
    public boolean validationOrder(SaleOrderResponseDTO saleOrderResponseDTO) throws ApplicationException {
        ...
        try {
            if (saleOrderResponseDTO.getPayAmount().compareTo(saleOrderResponseDTO.getAccrueAmount()) > -1) {
                return true;
            }
        }catch(Exception e){
