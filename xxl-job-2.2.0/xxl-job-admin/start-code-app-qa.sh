...
mv $proj_home/deepexi-gxs-middle-order-provider/target/deepexi-gxs-middle-order-*.jar $proj_home/deepexi-gxs-middle-order-provider/target/demo.jar # 兼容所有sh脚本
...
# 删除容器
docker rm -f deepexi.gxs.middle.order &>/dev/null
...
docker run -d --restart=on-failure:5 --privileged=true \
  ...
  --name deepexi.gxs.middle.order dd/deepexi.gxs.middle.order \
  java \
